...
class binning:
    ...
    def masterbinner(HJD,mag,magerr,Epoch,period,bins=40,weighted=True,norm_factor='alt',centered=True,pdot=0):
        ...
        ob_maglist=mag ; ob_magerr=magerr
        ...
        dphase=1/bins
        def makebin(phase):
            phases_in_bin=[] ; fluxes_in_bin=[] ; errors_in_bin=[]
            index_in_bin=[]
            for n in range(observations):
                if centered == False:
        ...
        binnedfluxlist=[] ;binnederrorlist=[] ; avgphaselist=[]
        master_phases_in_bin=[] ; master_fluxes_in_bin=[] ; master_errors_in_bin=[]
        master_index_in_bin=[]
        ...
        master_stuff_in_bin=[master_phases_in_bin,master_fluxes_in_bin,master_errors_in_bin,master_index_in_bin] # 5
    ...
    def masterbinner_FF(fileName,Epoch,period,bins,
        ...
        HJD=file[HJDcol] ; mag=file[magcol] ; magerr=file[magerrcol]
    ...
    def minipolybinner(c_master_phases,c_master_fluxes,nc_master_phases,nc_master_fluxes,
        ...
        for section in range(sections):
            if section == 0:
                ...
                c_polylist=calc.poly.polylist(c_coef,section/sections-halfsec,section/sections+halfsec,section_res)
    ...
    def polybinner(input_file,Epoch,period,sections=4,norm_factor='alt',
        ...
        if len(HJD_mag_magerr) == 3:
            c_MB=binning.masterbinner(HJD_mag_magerr[0],HJD_mag_magerr[1],HJD_mag_magerr[2],Epoch,period,sections,centered=True,norm_factor=norm_factor,pdot=pdot)
            nc_MB=binning.masterbinner(HJD_mag_magerr[0],HJD_mag_magerr[1],HJD_mag_magerr[2],Epoch,period,sections,centered=False,norm_factor=norm_factor,pdot=pdot)
        else:
            c_MB=binning.masterbinner_FF(input_file,Epoch,period,sections,centered=True,norm_factor=norm_factor,pdot=pdot)
            nc_MB=binning.masterbinner_FF(input_file,Epoch,period,sections,centered=False,norm_factor=norm_factor,pdot=pdot)
        
        c_master_phases=c_MB[5][0]   ; c_master_fluxes=c_MB[5][1]
        nc_master_phases=nc_MB[5][0] ; nc_master_fluxes=nc_MB[5][1]
        
        ob_phaselist=c_MB[0][1] ; ob_fluxlist=c_MB[1][1]
...
class FT: #Fourier transform
    ...
    def sumatphase(phase,order,a,b):
        ...
        return sum(a[:order+1:]*np.cos(2*np.pi*phase*orders)+b[:order+1:]*np.sin(2*np.pi*phase*orders))
    
    def deriv_sumatphase(phase,order,a,b):
        deriv_atphase=0
        for k in range(1,order+1):
            deriv_atphase+=(-b[k]*np.sin(2*np.pi*k*phase)-a[k]*np.cos(2*np.pi*k*phase))*4*np.pi**2*k**2
    ...
    def FT_plotlist(a,b,order,resolution):
        ...
        """
        phase=0 ; FTfluxlist=[] ; FTphaselist=[] ; FTderivlist=[]
        while(phase < 1):
            FTfluxlist.append(FT.sumatphase(phase,order,a,b))
    ...
    def int_sumatphase(a,b,phase,order):
        ...
        # print(nlist)
        return a[0]*phase+(0.5/np.pi)*sum((a[1:order+1:]*np.sin(2*np.pi*nlist*phase)-b[1:order+1:]*np.cos(2*np.pi*nlist*phase))/nlist)
        # return int_sum
    ...
    def int_unc_atphase(phase,a_unc,b_unc):
        ...
        for k in range(1,orderp1):
            int_unc+=((a_unc[k]/k)*np.sin(2*np.pi*phase*k))**2+((b_unc[k]/k)*np.cos(2*np.pi*phase*k))**2
...
class OConnell: #O'Connell effect
    ...
    def OER_FT_error(a,b,a_unc,b_unc,order):
        ...
        uncTop=np.sqrt((FT.int_unc_atphase(0.5,a_unc,b_unc))**2+(FT.int_unc_atphase(0,a_unc,b_unc))**2+(0.5*sigsum)**2)
        uncBot=np.sqrt((FT.int_unc_atphase(1,a_unc,b_unc))**2+(FT.int_unc_atphase(0.5,a_unc,b_unc))**2+(0.5*sigsum)**2)
        OER_unc=OER*np.sqrt((uncTop/Top)**2+(uncBot/Bot)**2)
        return OER,OER_unc
    
    def OER_FT_error_fixed(a,b,a_unc,b_unc,order):
        a=np.array(a) ; b=np.array(b)
        a_unc=np.array(a_unc) ; b_unc=np.array(b_unc)
        nlist=np.arange(order+1)[1::]
        A=0.5*sum(a[nlist])
        B=sum(b[nlist[::2]]/nlist[::2])/np.pi
    ...
    def LCA_error(binnedfluxlist,binnederrorlist):
        ...
        sAddlist=[]
        while(k<len(binnedfluxlist)/2):
            sAddlist.append(2*np.sqrt((sf[k]*(g[k]/(f[k]**2)-(g[k]**2)/(f[k]**3)))**2+(sg[k]*g[k]/(f[k]**2)-1/(f[k]))**2))
            k+=1
        return 'not finished'
    #==================================
    def LCA_FT(a,b,order,resolution):
        ...
        """
        import scipy
        Phi=np.linspace(0,0.5,resolution) ; no_a=np.zeros(len(a)) ; K2=[]
        for phase in Phi:
...
class Flower: #stuff from Flower 1996, Torres 2010
    class T:
        ...
        c=[3.97914510671409,-0.654992268598245,1.74069004238509,-4.60881515405716,6.79259977994447,-5.39690989132252,2.19297037652249,-0.359495739295671]
        def Teff(BV):
...
class plot:
    ...
    def multiplot(figsize=(8,8),dpi=256,height_ratios=[3,1],hspace=0,sharex=True,sharey=False,fig=None):
        ...
        axs=fig.subplots(len(height_ratios),sharex=sharex,sharey=sharey,gridspec_kw={'hspace':hspace,'height_ratios':height_ratios})
        return axs
    ...
    def value_resid_plot(ob_phase,ob_mag,synth_phase,synth_mag,resid,figsize=(8,8),dpi=512,
                         ...
                         usetex=False,synth_width=None,fontS=12,labelScale=1.2,tickScale=1,tickRatio=0.5): #X=major ticks, x=minor
        """
        ...
        value.set_ylim(bottom=max(allvalues)+plot.amp(allvalues)*0.05,top=min(allvalues)-plot.amp(allvalues)*0.06)
        ...
        value.tick_params(axis='x',which='major',length=bigtick,width=tickwidth,direction='in',top=True,bottom=False,labelsize=fontS)
        value.tick_params(axis='y',which='major',length=bigtick,width=tickwidth,direction='in',right=True,labelsize=fontS)
        value.tick_params(axis='x',which='minor',length=smalltick,width=tickwidth,direction='in',top=True,bottom=False)
        ...
        #residual plot
        resids.tick_params(axis='x',which='major',length=bigtick,width=tickwidth,direction='in',top=False,bottom=True,labelsize=fontS)
        resids.tick_params(axis='y',which='major',length=bigtick,width=tickwidth,direction='in',right=True,labelsize=fontS)
        resids.tick_params(axis='x',which='minor',length=smalltick,width=tickwidth,direction='in',top=False,bottom=True)
    ...
    def BVR_comphalves(aB,bB,aV,bV,aR,bR,order,resolution,fluxoff=0.2,save=False,outputName='noname.png',
        ...
        flux.tick_params(axis='x',which='major',length=8,width=tickwidth,direction='in',top=True,bottom=False,labelsize=numbersize)
        flux.tick_params(axis='y',which='major',length=8,width=tickwidth,direction='in',right=True,labelsize=numbersize)
        ...
        #residual plot
        resid.tick_params(axis='x',which='major',length=8,width=tickwidth,direction='in',top=False,bottom=True,labelsize=numbersize)
        resid.tick_params(axis='y',which='major',length=8,width=tickwidth,direction='in',right=True,labelsize=numbersize)
    ...
    def sm_format(ax, X=None, x=None, Y=None, y=None, Xsize=7, xsize=3.5, tickwidth=1,
        ...
        ax.tick_params(axis='x',which='major',length=Xsize,width=tickwidth,direction=xdirection,top=xtop,bottom=xbottom,labelsize=numbersize)
        ax.tick_params(axis='y',which='major',length=Xsize,width=tickwidth,direction=ydirection,right=yright,labelsize=numbersize,left=yleft)
        ax.tick_params(axis='x',which='minor',length=xsize,width=tickwidth,direction=xdirection,top=xtop,bottom=xbottom)
        ax.tick_params(axis='y',which='minor',length=xsize,width=tickwidth,direction=ydirection,right=yright,left=yleft)
        if spines == False:
...
class Roche:
    def Kopal_cyl(rho,phi,z,q):
        return 1/np.sqrt(rho**2+z**2)+q/(np.sqrt(1+rho**2+z**2-2*rho*np.cos(phi)))-q*rho*np.cos(phi)+0.5*(1+q)*rho**2
    ...
    def gen_Kopal_cyl_x(rho,phi,x,q,
        ...
        return 1/np.sqrt((xp+a1)**2+yp**2+zp**2)+q/np.sqrt((xp-a2)**2+yp**2+zp**2)+0.5*(1+q)*(xp**2+yp**2)-0.5*q**2/(1+q)
    
    def Kopal_xyz(x,y,z,q,xcm=0,ycm=0,zcm=0):
        xp=x-xcm ; yp=y-ycm ; zp=z-zcm
        return 1/np.sqrt((xp+q/(1+q))**2+yp**2+zp**2)+q/np.sqrt((xp-1/(1+q))**2+yp**2+zp**2)+0.5*(1+q)*(xp**2+yp**2)-0.5*q**2/(1+q)
    ...
    def Kopal_zero(rho,phi,z,q,Kopal,body='M1'):
        r=rho
        if body == 'M1':
            return 1/np.sqrt(r**2+z**2)+q/np.sqrt(1+r**2+z**2-2*r*np.cos(phi))-q*r*np.cos(phi)+0.5*(1+q)*r**2-Kopal
        elif body == 'M2':
            return q/np.sqrt(r**2+z**2)+1/np.sqrt(1+r**2+z**2+2*r*np.cos(phi))+r*np.cos(phi)+0.5*(1+q)*r**2+(1-q)/2-Kopal
        elif body == 'dM1':
            return -r/(r**2+z**2)**(3/2)-q*(r-np.cos(phi))/(1+r**2+z**2-2*r*np.cos(phi))**(3/2)-q*np.cos(phi)+(1+q)*r
        elif body == 'dM2':
            return -r*q/(r**2+z**2)**(3/2)-(r+np.cos(phi))/(1+r**2+z**2+2*r*np.cos(phi))**(3/2)+np.cos(phi)+(1+q)*r
        else:
